...
# Author: Jack Barry
#       
#       This script contains all the code to train a covariance leaning model on the DREAMS dataset. For a tutorial on this approach check  
#       CovNNet/Example; some code in this script may be vague or uncommented, so its worth familiarizing yourself with the example before
#       diving into this script. The main body of this script begins at line 1073 (subject to change). all lines before that contain 
#       functions or imports. Paths may need to be updated for your file structure; those that likely need updating are flagged with 
#       "-- Path Update --". The main section of this script contains four sections, "Load Dataset", "Define Hyperparameters", "Build and
...
if __name__ == "__main__":
    ...
    mass_path   = "/home/gjc7gx/CovNNet/Params/Other_mass_params.txt"           #   <<<<<<<<<<<<<<<<<<< -- Path Update -- <<<<<<<<<<<<<<<<<<<<<<
    ...
    X_groups = [                                        # Defines log-linear normalization for x_i
    ...
    # Load and create Y
    y1_path = "/home/gjc7gx/CovNNet/Params/Other_denProf_pts_params.txt"        #   <<<<<<<<<<<<<<<<<<< -- Path Update -- <<<<<<<<<<<<<<<<<<<<<<
    Y1 = np.loadtxt(y1_path)[:, 1:]  # [N, 50]

    y2_path = "/home/gjc7gx/CovNNet/Params/rotCurve_pts_params.txt"             #   <<<<<<<<<<<<<<<<<<< -- Path Update -- <<<<<<<<<<<<<<<<<<<<<<
    ...
    MODEL_DIR  = "/home/gjc7gx/CovNNet/Models" # define path to save best model     <<<<<<<<<<<<<<<<<<< -- Path Update -- <<<<<<<<<<<<<<<<<<<<<<
    ...
    diag_eps = 1e-6     # avoid div by zero in diagonal 
    ...
    n_epochs = 35000     # number of training epochs (from personal experiance I find I need to train for longer than I think)
    ...
    if train:
        ...
        for epoch in range(1, n_epochs + 1):
            ...
            if epoch % 100 == 0:
                ...
                plt.savefig("/home/gjc7gx/CovNNet/Figures/ALLloss_curve.png", dpi=160) # <<<<<<<<<<<<<<<<<<< -- Path Update -- <<<<<<<<<<<<<<<<<<<<<<
    ...
    if plot:
        ...
        _ = plot_one_simulation_with_bands_and_corr_blocks(
                ...
                ci_sigma=2.0,
                savepath="/home/gjc7gx/CovNNet/Figures/ALL_one_sim_with_bands.png", # <<<<<<<<<<<<<<<<<<< -- Path Update -- <<<<<<<<<<<<<<<<<<<<<<
                basis_meta=basis_meta                 # pass basis meta
        )

        sweep_idx = 2 # defines j to sweep in x_j
        ...
        _ = animate_sweep_one_feature(
                ...
                fps=20,
                savepath_gif=f"/home/gjc7gx/CovNNet/Figures/sweep_x{sweep_idx}.gif", # <<<<<<<<<<<<<<<<<<< -- Path Update -- <<<<<<<<<<<<<<<<<<<<<<
                title_prefix=f"Sweep X[{sweep_idx}]"
        )
