...
if __name__ == "__main__":
    ...
    def _predict_bins_from_xn(mean_net, xn_vec, device, key_y, basis_meta):
        ...
        Returns: (mu1, mu2) where mu1 = density profile, mu2 = rotation profile
        """
        mean_net.eval()
        xb = torch.from_numpy(xn_vec.astype(np.float32)).to(device).unsqueeze(0)  # [1,dx]
    ...
    # ------------------------------------------------------------------------------
    # Theoretical-model inversion from NN mean/cov (σ is a FITTED parameter)
    # ------------------------------------------------------------------------------
    ...
    def _denorm_feature_scalar(xn_scalar, key_x, j):
        """Map normalized X[j] back to raw using key_x['mean'][j], key_x['std'][j]."""
        try:
            mu = float(key_x["mean"][j])
            sd = float(key_x["std"][j])
            return mu + sd * float(xn_scalar)
        except Exception:
            return float(xn_scalar)

    # ---------------------- Hypothesis/prior configuration ------------------------
    @dataclass
    class PriorConfig:
        # μ prior: μ ≈ b0 + b1 * X2_raw (X2_raw is de-normalized from Xn via key_x)
        use_mu_prior: bool = False
        b0: float = 0.0
        b1: float = 0.0
        x2_index: int = 2
        lambda_mu: float = 0.0  # weight (residual uses sqrt(lambda))

        # A prior: A ≈ a0 + a1 * V (V is any visible-matter scalar you pass in)
    ...
    def v_rotation_model_stable(r, rho0, rs, gamma, beta, A, mu, sigma, G=4.30091e-6):
        ...
        bump = A / (r_safe * sigma_safe * np.sqrt(2.0 * np.pi)) * np.exp(-0.5 * ((log_r - mu) / sigma_safe) ** 2)
        v = v_grav + bump
        return np.nan_to_num(v, nan=0.0, posinf=1e300)

    # ----------------------------- Parameter handling -----------------------------
    def sanitize_theta(theta, r, mode="fit", sigma_bounds=(0.15, 3.5)):
        """
        Enforce safe/physical ranges.
        ...
        if mode == "fit":
            ...
            sigma = float(np.clip(sigma, sig_lo, sig_hi))
        else:
    ...
    # ----------------------------- Fit θ̂ to NN mean ------------------------------
    def _ensure_strict_bounds(lb, ub, eps=1e-12):
        """Return (lb, ub) with lb < ub elementwise; nudges with tiny eps if needed."""
        lb = np.asarray(lb, dtype=np.float64).copy()
        ub = np.asarray(ub, dtype=np.float64).copy()
        bad = lb >= ub
        if np.any(bad):
            scale = np.maximum(1.0, np.maximum(np.abs(lb[bad]), np.abs(ub[bad])))
            delta = eps * scale
            lb[bad] = ub[bad] - delta
        return lb, ub

    def fit_theta_to_mu(
        mu_bins, Sigma_bins, r, theta_init, bounds=None,
        *, prior_cfg: PriorConfig | None = None,
        xn_vec: np.ndarray | None = None, key_x: dict | None = None,
        V_value: float | None = None
    ):
        """
        Whitened LS with robust loss and optional priors:
        - μ ≈ b0 + b1 * X2_raw  (X2_raw from xn_vec/key_x)
        - A ≈ a0 + a1 * V
        θ = [rho0, rs, gamma, beta, A, mu, sigma]
        """
        ydim = mu_bins.size
        ...
        # μ prior target (optional)
        mu_prior_val, mu_w = None, 0.0
        if prior_cfg is not None and prior_cfg.use_mu_prior:
            if xn_vec is None or key_x is None:
                raise ValueError("μ prior needs xn_vec and key_x to de-normalize X2.")
            x2_raw = _denorm_feature_scalar(xn_vec[prior_cfg.x2_index], key_x, prior_cfg.x2_index)
            mu_prior_val = prior_cfg.b0 + prior_cfg.b1 * x2_raw
            mu_w = np.sqrt(max(0.0, prior_cfg.lambda_mu))

        # A prior target (optional)
        A_target, A_w = None, 0.0
        if prior_cfg is not None and prior_cfg.use_A_prior and V_value is not None:
            A_target = prior_cfg.a0 + prior_cfg.a1 * V_value
            A_w = np.sqrt(max(0.0, prior_cfg.lambda_A))

        def resid(theta):
            theta_use = np.array(theta, dtype=np.float64, copy=True)
            y_model = stacked_theory_profiles(theta_use, r, mode="fit")
            base = Winv @ (np.clip(y_model, -1e12, 1e12) - np.clip(mu_bins, -1e12, 1e12))
            parts = [base]
            if A_target is not None and A_w > 0.0:
                parts.append(A_w * (theta_use[IDX_A] - A_target))
            if mu_prior_val is not None and mu_w > 0.0:
                parts.append(mu_w * (theta_use[IDX_MU] - mu_prior_val))
        ...
        if bounds is not None:
            ...
            lb = np.array([1e-12, 1e-3, 0.0,  2.0,   -1e6,  np.log(r.min())-5.0, 0.15], dtype=np.float64)
            ub = np.array([1e20,  1e3,  2.0,  6.0,    1e6,  np.log(r.max())+5.0, 3.50], dtype=np.float64)
            lb, ub = _ensure_strict_bounds(lb, ub, eps=1e-12)
        # floors
        lb[:2] = np.maximum(lb[:2], [1e-12, 1e-3])
    ...
    def finite_diff_jacobian(theta_hat, r, eps_rel=None):
        ...
        if eps_rel is None:
            eps_rel = np.array([1e-4, 1e-4, 5e-4, 5e-4, 1e-4, 1e-4, 1e-3], dtype=np.float64)
    ...
    @torch.no_grad()
    def infer_theory_params_for_xn(
        mean_net, cov_net, xn_vec, r_tags, device, key_y, basis_meta,
        theta_init=None, bounds=None, print_prefix="[theory] ",
        *, prior_cfg: PriorConfig | None = None, key_x: dict | None = None, V_value: float | None = None
    ):
        ...
        # default init/bounds for 7 params
        if theta_init is None:
            rho0_guess = max(1.0, np.median(mu_bins[:len(r)]))
            rs_guess   = 5.0
            theta_init = np.array([rho0_guess, rs_guess, 1.0, 3.0, 50.0, np.log(np.median(r)), 1.0],
                                dtype=np.float64)
        ...
        theta_hat = fit_theta_to_mu(
            mu_bins, Sigma_bins, r, theta_init, bounds=bounds,
            prior_cfg=prior_cfg, xn_vec=xn_vec, key_x=key_x, V_value=V_value
        )
        ...
        return theta_hat, Sigma_theta, Corr_theta, J, mu_bins, Sigma_bins

    # ---- Example call on mean normalized X ----
    x_mean_norm = Xn.mean(axis=0).astype(np.float32)
    prior_cfg = PriorConfig(
        use_mu_prior=False, b0=0.0, b1=0.0, x2_index=2, lambda_mu=0.0,
        use_A_prior=False, a0=0.0, a1=0.0, lambda_A=0.0,
    )
    V_example = None  # set to a scalar if you want the A prior

    theta_hat, Sigma_theta, Corr_theta, J, mu_bins, Sigma_bins = infer_theory_params_for_xn(
        mean_net, cov_net,
        xn_vec=x_mean_norm,
        r_tags=r_tags,
        device=device,
    ...
    def sample_model_profile_distribution(theta_hat, Sigma_theta, r, n_samples=5000, seed=42,
        ...
        """
        Sample θ ~ N(θ̂, Σθ) robustly; return empirical mean/cov/corr in y-space.
        (θ has 7 dims, including σ).
        """
    ...
    # --------------------------------------------------------------------------
    # θ–X correlations via *local* sampling in X-space (mean X + perturbations)
    # σ is FITTED (part of θ) during each inversion.
    # --------------------------------------------------------------------------
    @torch.no_grad()
    def estimate_theta_X_corr_local_perturb(
        mean_net,
        cov_net,
        Xn,                         # normalized dataset features [N, p]
        r_tags,                     # radii (length M)
        device,
        key_y,                      # coeff/bins key (for _predict_bins_and_cov_from_xn)
        basis_meta,                 # basis meta used by _predict_bins_and_cov_from_xn
        *,
        n_samples: int = 300,       # how many local X draws
        step_sigma: float = 0.15,   # Gaussian step size in *normalized* units
        clamp_lo: float = 0.02,     # keep samples away from exact edges
        clamp_hi: float = 0.98,
        theta_init=None,
        bounds=None,
        save_dir="/home/gjc7gx/CovNNet/TheoryFit",
    ):
        """
        Build Corr(θ, X) by drawing X' ~ N(mean(Xn), step_sigma^2 I) in normalized space,
        clamped to [clamp_lo, clamp_hi], then inverting each draw to θ (including σ).
        Returns Corr_theta_X, Corr_theta, Corr_X, theta_names, x_labels, Theta, Xused.
        """
        os.makedirs(save_dir, exist_ok=True)
        rng = np.random.default_rng(0)

        # Reference point: dataset mean in normalized space
        x0 = Xn.mean(axis=0).astype(np.float32)     # [p]
        p  = x0.size
        r  = np.asarray(r_tags, dtype=np.float64)

        # Precompute reasonable init/bounds if not given (7 params)
        if bounds is None:
            lb = np.array([1e-12, 1e-3, 0.0,  2.0,   -1e6,  np.log(r.min())-5.0, 0.15], dtype=np.float64)
            ub = np.array([1e20,  1e3,  2.0,  6.0,    1e6,  np.log(r.max())+5.0, 3.50], dtype=np.float64)
            lb, ub = _ensure_strict_bounds(lb, ub, eps=1e-12)
        ...
        def _one_inversion(xn_vec, theta_init_local):
            mu_bins, Sigma_bins = _predict_bins_and_cov_from_xn(
                mean_net, cov_net, xn_vec, device, key_y, basis_meta, Y1_len=len(r_tags)
            )
            theta_hat = fit_theta_to_mu(
                mu_bins, Sigma_bins, r, theta_init_local, bounds=bounds,
                prior_cfg=None, xn_vec=None, key_x=None, V_value=None
            )
            return theta_hat, mu_bins, Sigma_bins

        # draw samples around mean(Xn)
        draws = [x0.copy()]
        if n_samples > 1:
            noise = rng.normal(loc=0.0, scale=step_sigma, size=(n_samples-1, p)).astype(np.float32)
            Xcand = np.clip(x0[None, :] + noise, clamp_lo, clamp_hi)
            draws += [Xcand[i] for i in range(Xcand.shape[0])]

        # loop: invert each draw
        theta_init_running = None
        for k, xn_vec in enumerate(draws):
            xn_vec = xn_vec.astype(np.float32)
            if (theta_init_running is None) or (theta_init is not None and k == 0):
                if theta_init is None:
                    mu_bins0, _Sigma0 = _predict_bins_and_cov_from_xn(
                        mean_net, cov_net, xn_vec, device, key_y, basis_meta, Y1_len=len(r_tags)
                    )
                    rho0_guess = max(1.0, np.median(mu_bins0[:len(r)]))
                    rs_guess   = 5.0
                    theta_init_running = np.array(
                        [rho0_guess, rs_guess, 1.0, 3.0, 50.0, np.log(np.median(r)), 1.0],
                        dtype=np.float64
                    )
                else:
                    theta_init_running = np.asarray(theta_init, dtype=np.float64)

            theta_hat, _mu_bins, _Sigma_bins = _one_inversion(xn_vec, theta_init_running)
            theta_init_running = theta_hat.copy()  # warm start

            thetas.append(theta_hat)
            xdraws.append(xn_vec)

        Theta = np.vstack(thetas)                 # [n, 7]
        Xused = np.vstack(xdraws).astype(np.float64)  # [n, p]
        ...
        n = Tz.shape[0]
        Corr_theta_X = (Tz.T @ Xz) / max(1, (n - 1))      # [7, p]
        Corr_theta   = (Tz.T @ Tz) / max(1, (n - 1))      # [7, 7]
        Corr_X       = (Xz.T @ Xz) / max(1, (n - 1))      # [p, p]
        ...
        try:
            ...
            if len(x_labels) != p: raise ValueError
        except Exception:
            x_labels = [f"X{j}" for j in range(p)]

        # Save quick artifacts
        np.savetxt(os.path.join(save_dir, "theta_x_corr_local.txt"), Corr_theta_X)
        np.savetxt(os.path.join(save_dir, "theta_corr_local.txt"),    Corr_theta)
        np.savetxt(os.path.join(save_dir, "x_corr_local.txt"),        Corr_X)
        np.savetxt(os.path.join(save_dir, "theta_samples_local.txt"), Theta, header="rho0 rs gamma beta A mu sigma")
        np.savetxt(os.path.join(save_dir, "xdraws_local.txt"),        Xused, header="normalized X draws")

        print(f"[local] Built Corr(θ, X) from {n_samples} local X-samples around mean(Xn). Saved to: {save_dir}")

        return Corr_theta_X, Corr_theta, Corr_X, theta_names, x_labels, Theta, Xused

    # ---------------------- Example local-perturb call ----------------------
    Corr_theta_X_local, Corr_theta_ds, Corr_X, theta_names, x_labels, Theta_local, Xused_local = \
        estimate_theta_X_corr_local_perturb(
            mean_net, cov_net,
            Xn=Xn,
            r_tags=r_tags,
            device=device,
    ...
    # (Optional) quick visualization for Corr(θ,X)
    def _quick_show_thetaX(C, theta_names, x_labels, title="Corr(θ, X) — local samples"):
        fig, ax = plt.subplots(figsize=(10, 4), constrained_layout=True)
        im = ax.imshow(C, vmin=-1, vmax=1, origin="lower", aspect="auto")
        ax.set_yticks(np.arange(len(theta_names))); ax.set_yticklabels(theta_names)
        ax.set_xticks(np.arange(len(x_labels)));    ax.set_xticklabels(x_labels, rotation=45, ha="right")
        ax.set_title(title)
        cbar = fig.colorbar(im, ax=ax, fraction=0.046, pad=0.04); cbar.set_label("corr")
        fp = "/home/gjc7gx/CovNNet/TheoryFit/corr_thetaX_local.png"
        plt.savefig(fp, dpi=160); plt.close(fig)
        print(f"[local] Saved: {fp}")

    _quick_show_thetaX(Corr_theta_X_local, theta_names, x_labels)
    ...
    def plot_nn_vs_model_profiles_and_corr(
        ...
        ax5.set_yticks(np.arange(len(row_labels))); ax5.set_yticklabels(row_labels)
        ax5.set_xticks(np.arange(len(col_labels))); ax5.set_xticklabels(col_labels, rotation=45, ha="right")
        ax5.axhline(rsplit, color="k", lw=0.8); ax5.axvline(csplit, color="k", lw=0.8)
    ...
    # ------------------------------------------------------------------
    # 1. Build dataset-level correlations (θ–X, θ–θ, X–X) via local X sampling
    # ------------------------------------------------------------------
    Corr_theta_X_local, Corr_theta_ds, Corr_X, theta_names, x_labels, Theta_local, Xused_local = \
        estimate_theta_X_corr_local_perturb(
            mean_net, cov_net,
    ...
    plot_nn_vs_model_profiles_and_corr(
        ...
        Corr_theta_X=Corr_theta_X_local,
    ...
    # ------------------------------------------------------------------------------
    # X–θ correlations via local X sampling around mean(Xn), feeding the NN, then
    # fitting θ (including σ) to each generated profile (sample→fit version).
    # ------------------------------------------------------------------------------
    @torch.no_grad()
    def estimate_theta_X_corr_sample_then_fit(
        mean_net,
        cov_net,
        Xn,                         # normalized dataset features [N, p]
        r_tags,                     # radii (length M)
        device,
        key_y,                      # coeff/bin key for _predict_bins_and_cov_from_xn
        basis_meta,                 # basis meta for _predict_bins_and_cov_from_xn
        *,
        n_samples: int = 300,       # number of local X draws
        step_sigma: float = 0.15,   # Gaussian step size in normalized units
        clamp_lo: float = 0.02,     # keep samples inside (lo,hi) to avoid edges
        clamp_hi: float = 0.98,
        theta_init=None,
        bounds=None,
        save_dir="/home/gjc7gx/CovNNet/TheoryFit"
        ...
        """
        Draw X' ~ N(mean(Xn), step_sigma^2 I) in normalized space, clamp to [clamp_lo, clamp_hi],
        feed each draw through the NN to get (mu_bins, Sigma_bins), and fit full θ (incl. σ).
        Returns Corr(θ, X), Corr(θ, θ), Corr(X, X) plus labels and the actual samples used.
        """
        os.makedirs(save_dir, exist_ok=True)
        rng = np.random.default_rng(0)

        # Setup & defaults
        x0 = Xn.mean(axis=0).astype(np.float32)  # [p] normalized mean
        p  = x0.size
        r  = np.asarray(r_tags, dtype=np.float64)

        # Safe default bounds for 7 parameters
        if bounds is None:
            lb = np.array([1e-12, 1e-3, 0.0,  2.0,   -1e6,  np.log(r.min())-5.0, 0.15], dtype=np.float64)
            ub = np.array([1e20,  1e3,  2.0,  6.0,    1e6,  np.log(r.max())+5.0, 3.50], dtype=np.float64)
            lb, ub = _ensure_strict_bounds(lb, ub, eps=1e-12)
        ...
        # Construct draws: first the exact mean; then Gaussian around it (clamped)
        draws = [x0.copy()]
        if n_samples > 1:
            noise = rng.normal(loc=0.0, scale=step_sigma, size=(n_samples-1, p)).astype(np.float32)
            Xcand = np.clip(x0[None, :] + noise, clamp_lo, clamp_hi)
        ...
        for k, xn_vec in enumerate(draws):
            ...
            if (theta_init_running is None) or (theta_init is not None and k == 0):
                if theta_init is None:
                    # Init from the current NN profile at this X
                    mu_bins0, _Sigma0 = _predict_bins_and_cov_from_xn(
                        mean_net, cov_net, xn_vec, device, key_y, basis_meta, Y1_len=len(r_tags)
                    )
                    rho0_guess = max(1.0, np.median(mu_bins0[:len(r)]))
            ...
            theta_hat, _mu_bins, _Sigma_bins = _one_inversion(xn_vec, theta_init_running)
            theta_init_running = theta_hat.copy()  # warm-start next draw
        ...
        np.savetxt(os.path.join(save_dir, "Corr_theta_X.txt"), Corr_theta_X)
        np.savetxt(os.path.join(save_dir, "Corr_theta.txt"),   Corr_theta)
        np.savetxt(os.path.join(save_dir, "Corr_X.txt"),       Corr_X)
        np.savetxt(os.path.join(save_dir, "Theta_samples.txt"),Theta, header="rho0 rs gamma beta A mu sigma")
        np.savetxt(os.path.join(save_dir, "Xdraws_norm.txt"),  Xused, header="normalized X draws")

        print(f"[X-θ] Built Corr(θ, X) from {n_samples} local X-samples around mean(Xn). Saved to: {save_dir}")
        return Corr_theta_X, Corr_theta, Corr_X, theta_names, x_labels, Theta, Xused


    # ---------------------- Example call (drop-in) ----------------------
    Corr_theta_X, Corr_theta_ds2, Corr_X2, theta_names2, x_labels2, Theta_local2, Xused_local2 = \
        estimate_theta_X_corr_sample_then_fit(
            mean_net, cov_net,
            Xn=Xn,
            r_tags=r_tags,
            device=device,
            key_y=Yn_key,
            basis_meta=basis_meta,
            n_samples=300,       # tweak as desired
            step_sigma=0.12,     # normalized step size
            clamp_lo=0.02,
            clamp_hi=0.98,
            theta_init=None,
            bounds=None,
    ...
    # (Optional) quick heatmap of Corr(θ, X)
    def _quick_show_thetaX(C, theta_names, x_labels, title="Corr(θ, X) — sample→fit"):
        fig, ax = plt.subplots(figsize=(10, 4), constrained_layout=True)
        im = ax.imshow(C, vmin=-1, vmax=1, origin="lower", aspect="auto")
        ax.set_yticks(np.arange(len(theta_names))); ax.set_yticklabels(theta_names)
        ax.set_xticks(np.arange(len(x_labels)));    ax.set_xticklabels(x_labels, rotation=45, ha="right")
        ax.set_title(title)
        cbar = fig.colorbar(im, ax=ax, fraction=0.046, pad=0.04); cbar.set_label("corr")
        fp = "/home/gjc7gx/CovNNet/TheoryFit/corr_thetaX_samplefit.png"
        plt.savefig(fp, dpi=160); plt.close(fig)
        print(f"[X-θ] Saved: {fp}")

    _quick_show_thetaX(Corr_theta_X, theta_names2, x_labels2)

    # ======================================================================
    # Sweep each input X_j (5th→95th pct), feed NN, fit θ (incl. σ) to the NN
    # profile, and plot each coefficient vs the swept input (one figure/feature).
    # ======================================================================
    @torch.no_grad()
    def sweep_and_plot_theta_vs_each_X(
        mean_net,
        cov_net,
        Xn,                 # normalized inputs [N, p]
        r_tags,             # radii
        device,
        key_y,              # for _predict_bins_and_cov_from_xn
        basis_meta,         # for _predict_bins_and_cov_from_xn
        *,
        key_x=None,         # if provided: de-normalize x-axis values with key_x['mean'],['std']
        feature_names=None, # optional list[str] of length p; else falls back to X_cols or X0..Xp-1
        grid_points=25,
        out_dir="/home/gjc7gx/CovNNet/TheoryFit/ThetaVsX",
        theta_init=None,
        bounds=None
    ):
        os.makedirs(out_dir, exist_ok=True)
        ...
        # Default bounds for 7 parameters (incl. σ)
        if bounds is None:
            lb = np.array([1e-12, 1e-3, 0.0,  2.0,   -1e6,  np.log(r.min())-5.0, 0.15], dtype=np.float64)
            ub = np.array([1e20,  1e3,  2.0,  6.0,    1e6,  np.log(r.max())+5.0, 3.50], dtype=np.float64)
            lb, ub = _ensure_strict_bounds(lb, ub, eps=1e-12)
        ...
        # Base vector: dataset mean in normalized space
        x_mean = Xn.mean(axis=0).astype(np.float32)

        # LaTeX-style names for θ on plots (7)
        theta_names_plot = [r"$\rho_0$", r"$r_s$", r"$\gamma$", r"$\beta$", r"$A$", r"$\mu$", r"$\sigma$"]

        for j in range(p):
            # Build sweep in normalized space (5th→95th percentile of Xn[:, j])
            p5, p95 = np.percentile(Xn[:, j], [5, 95])
            xj_grid_norm = np.linspace(p5, p95, grid_points, dtype=np.float32)

            # De-normalized x-axis values for nicer labels (if key_x provided)
            if key_x is not None:
                xj_grid_raw = np.array([_denorm_feature_scalar(val, key_x, j) for val in xj_grid_norm], dtype=np.float64)
                x_axis_vals = xj_grid_raw
                x_label = f"{feature_names[j]} (raw)"
            else:
                x_axis_vals = xj_grid_norm.astype(np.float64)
                x_label = f"{feature_names[j]} (normalized)"

            # Storage for fitted coefficients across the sweep
            Theta_sweep = np.zeros((grid_points, 7), dtype=np.float64)

            # Warm start with either provided theta_init or an NN-informed guess at first point
            theta_warm = None

            for k, xj_val in enumerate(xj_grid_norm):
                # Build input vector: mean everywhere, except X[j] = grid value
                xn_vec = x_mean.copy()
                xn_vec[j] = xj_val

                # NN → (mu_bins, Sigma_bins)
                mu_bins, Sigma_bins = _predict_bins_and_cov_from_xn(
                    mean_net, cov_net, xn_vec, device, key_y, basis_meta, Y1_len=len(r_tags)
                )

                # Make an initial guess if needed
                if theta_warm is None:
                    if theta_init is None:
                        rho0_guess = max(1.0, np.median(mu_bins[:len(r)]))
                        rs_guess   = 5.0
                        theta_warm = np.array([rho0_guess, rs_guess, 1.0, 3.0, 50.0, np.log(np.median(r)), 1.0],
                                            dtype=np.float64)
                    else:
                        theta_warm = np.asarray(theta_init, dtype=np.float64)

                # Fit θ (incl. σ) to the NN mean profile
                theta_hat = fit_theta_to_mu(
                    mu_bins, Sigma_bins, r, theta_warm, bounds=bounds,
                    prior_cfg=None, xn_vec=None, key_x=None, V_value=None
                )

                Theta_sweep[k, :] = theta_hat
                theta_warm = theta_hat.copy()  # warm-start next grid point

            # ---------- Plot: one figure per feature with 7 subplots ----------
            fig, axes = plt.subplots(3, 3, figsize=(12, 8.5), constrained_layout=True)
            axes = axes.ravel()
            for idx in range(7):
                ax = axes[idx]
                ax.plot(x_axis_vals, Theta_sweep[:, idx], lw=2)
                ax.set_xlabel(x_label)
                ax.set_ylabel(theta_names_plot[idx])
                ax.grid(True, alpha=0.3)
            # Hide the two extra empty panels
            for idx in range(7, 9):
                axes[idx].axis('off')
            fig.suptitle(f"Fitted coefficients vs {feature_names[j]} (5th→95th pct sweep)")

            # Save
            safe_name = feature_names[j].replace("/", "_").replace(" ", "_")
            out_png = os.path.join(out_dir, f"theta_vs_{safe_name}.png")
            plt.savefig(out_png, dpi=160)
            plt.close(fig)
            print(f"[sweep] Saved: {out_png}")

            # Optional: also save the numeric table
            out_txt = os.path.join(out_dir, f"theta_vs_{safe_name}.txt")
            header_cols = " ".join([name.replace("$","").replace("\\","") for name in theta_names_plot])
            np.savetxt(out_txt,
                    np.column_stack([x_axis_vals, Theta_sweep]),
                    header=f"{feature_names[j]}  {header_cols}")
            print(f"[sweep] Saved: {out_txt}")

    # ----------------- Run the sweep & plot for your current session -----------------
    sweep_and_plot_theta_vs_each_X(
        mean_net=mean_net,
        cov_net=cov_net,
        Xn=Xn,
        r_tags=r_tags,
        ...
        key_y=Yn_key,
        basis_meta=basis_meta,
        key_x=Xn_key,                 # so x-axis is in raw units; omit to keep normalized
        feature_names=None,           # or pass a list like X_cols
        grid_points=25,
        out_dir="/home/gjc7gx/CovNNet/TheoryFit/ThetaVsX",
        theta_init=None,              # let the routine guess from the first profile
        bounds=None                   # use safe defaults
    )
