...
# Author: Jack Barry
#       
#       This script contains all the code to train a standard MLP model on the DREAMS dataset in order to predict TWO profiles concurrently.
#       The path definitions for Y1 and Y2 are the same as those defined in Cov_main.py. Some file paths may need to change. To find those
#       likely to need changing, Ctrl+f "gjc7gx". The main section of this script contains four sections, "Load Dataset", "Define 
#       Hyperparameters", "Build and Train Models", and "Plot". To load a pre trained model, set train = False (at the top of main), and 
#       change the model name and structure that of the model you want to load in "Define Hyperparameters" (model structure should be included
#       in the model name in the form NAME_{cov_hc}_{cov_nl}_{mean_hc}_{mean_nl}.pt. This is not done automatically, so be sure to update 
...
def reconstruct_profile_from_output(y_vec: np.ndarray,
                                    ...
                                    basis_meta: dict,
                                    target_block: str,
                                    r_tags: np.ndarray) -> np.ndarray:
    """
    Take a single normalized joint output y_vec = [Y1_part, Y2_part],
    denormalize, and return ONE 50-bin profile for the requested block
    ('Y1' or 'Y2').
    """
    # 1) denormalize from [0,1] back to original coefficient/bin scale
    ...
    if use_basis:
        # --- Triangular-basis case: y_denorm are coeffs for Y1 and Y2 ---
        k1 = basis_meta["y1"]["k"]               # number of Y1 coeffs
        Phi1 = basis_meta["y1"]["Phi"]           # [50, k1]
        Phi2 = basis_meta["y2"]["Phi"]           # [50, k2]
        log1 = basis_meta["y1"]["logspace"]
        log2 = basis_meta["y2"]["logspace"]

        coeffs_y1 = y_denorm[:k1]
        coeffs_y2 = y_denorm[k1:k1 + Phi2.shape[1]]

        if target_block == "Y1":
            profile = Phi1 @ coeffs_y1
            if log1:
                profile = np.exp(profile)
        elif target_block == "Y2":
            profile = Phi2 @ coeffs_y2
            if log2:
                profile = np.exp(profile)
        else:
        ...
        # --- Raw-bin case: assume [Y1_bins(0:nbins), Y2_bins(nbins:2*nbins)] ---
        n_bins = len(r_tags)  # e.g. 50
        if target_block == "Y1":
            return y_denorm[:n_bins]
        elif target_block == "Y2":
            return y_denorm[n_bins:2 * n_bins]
        else:
            raise ValueError(f"Unknown target_block={target_block}, must be 'Y1' or 'Y2'.")
...
def plot_one_test_example(mean_net,
                        ...
                        basis_meta,
                        r_tags,
                        target_block: str,     # "Y1" or "Y2"
                        use_tri_basis: bool,   # kept for signature compatibility
                        device,
                        yscale: str = "linear"):
    """
    Plot the first sample in the test_loader:
    - True profile vs predicted profile for the chosen block (Y1 or Y2)
    - y-axis can be 'linear' or 'log'
    """
    mean_net.eval()

    # Grab first batch and first element
    xb, yb = next(iter(test_loader))   # xb: [B, X_len], yb: [B, Y_len] (normalized joint Y1+Y2)
    xb = xb.to(device)
    yb = yb.to(device)
    ...
    # Reconstruct physical profiles (50 bins on r_tags) for the requested block
    true_profile = reconstruct_profile_from_output(
        y_true_0, Yn_key, basis_meta, target_block, r_tags
    )
    pred_profile = reconstruct_profile_from_output(
        y_pred_0, Yn_key, basis_meta, target_block, r_tags
    )
...
def plot_all_test_profiles_and_error(mean_net,
    ...
    for xb, yb in test_loader:
        ...
        with torch.no_grad():
            y_pred = mean_net(xb)  # [B, Y_len joint]
        ...
        B = y_true_np.shape[0]
        for i in range(B):
            # Reconstruct 50-bin profiles for the requested block
            true_profile = reconstruct_profile_from_output(
                y_true_np[i], Yn_key, basis_meta, target_block, r_tags
            )
            pred_profile = reconstruct_profile_from_output(
                y_pred_np[i], Yn_key, basis_meta, target_block, r_tags
            )

            # Plot with low alpha so many curves can be overlaid
            ax.plot(r_tags, pred_profile / true_profile,
    ...
    ax.set_title(
        ...
        f"(pred/true curves, yscale={yscale})"
    )
    ax.grid(True, which="both", linestyle=":")
    ...
    print(f"[metric] mean |log(pred/true)| over test set ({target_block}) = {mean_abs_log:.6g}")
    return mean_abs_log
...
def save_test_profiles_txt(mean_net,
    ...
    for the requested block (Y1 or Y2).
    ...
    print(f"[save] Saved {target_block} test profiles to:\n{out_path}")
...
if __name__ == "__main__":
    ...
    if USE_TRI_BASIS:
        ...
        log_r = np.log(r_tags)
        knots1 = np.linspace(log_r.min(), log_r.max(), K1_COEFFS)
        knots2 = np.linspace(log_r.min(), log_r.max(), K2_COEFFS)

        Phi1 = triangular_basis_logspace(log_r, knots1, add_bias=True)
        Phi2 = triangular_basis_logspace(log_r, knots2, add_bias=True)

        C1 = solve_coeffs(Y1, Phi1, logspace=LOG_Y1_BASIS)
        C2 = solve_coeffs(Y2, Phi2, logspace=LOG_Y2_BASIS)

        # ---- NEW: learn BOTH ----
        Y = np.concatenate([C1, C2], axis=1)

        Y_groups = [
            {"cols": list(range(0, C1.shape[1])), "mode": "linear"},         # Y1 coeffs
            {"cols": list(range(C1.shape[1], Y.shape[1])), "mode": "linear"} # Y2 coeffs
        ]
        ...
        # ---- NEW: learn BOTH raw 50-bin outputs ----
        Y = np.concatenate([Y1, Y2], axis=1)  # [N, 100]

        Y_groups = [
            {"cols": list(range(0, 50)), "mode": "log"},     # Y1
            {"cols": list(range(50, 100)), "mode": "linear"} # Y2
        ]

        Yn, Yn_key = norm_params(Y, Y_groups, margin_pct=0.02)
        Y_len = Y.shape[1]

        basis_meta.update({"use_basis": False})
    ...
    if plot:
        # Example: plot Y1 and Y2 test examples
        for block in ["Y1", "Y2"]:
            ...
            print(f"[metric] Final mean |log(pred/true)| for {block} = {mean_abs_log:.6g}")

        # Two separate TXT outputs: one for Y1, one for Y2
        out_txt_y1 = "/home/gjc7gx/CovNNet/Params/Y1O_STO_PredTrue.txt"
        out_txt_y2 = "/home/gjc7gx/CovNNet/Params/Y2O_STO_PredTrue.txt"
